...
module.exports = {
  ...
  installInto: expect => {
    expect.preferredOutputWidth = 80;
    ...
    expect.addAssertion('<any> to be a mongo id', (expect, subject) => {
      // not using ObjectId.isValid('foo') since it allows any 12-byte string
      return expect(subject, 'to match', /^[a-fA-F0-9]{24}$/);
    });
